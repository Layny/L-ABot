    ...
    run: (message, args, client) => {
        if (args[0]) {
            ...
            message.channel.send(new Discord.MessageEmbed()
                ...
                .setDescription(`${client.commands.filter(command => command.help).map(command => `\`${config.prefix}${command.name}\``).join(' ')}\n\nPour plus d'information sur une commande spécifique, tapez \`${config.prefix}help [nom de la commande]\``))
        }
    },
    name: 'help',
    help: {
        description: 'Cette commande permet d\'avoir la fonctionnatilité de chaque commande utilisable',
